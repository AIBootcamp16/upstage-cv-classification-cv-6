...
class Config:
    ...
    def __init__(self):
        ...
        self.MODEL_NAME = 'tf_efficientnet_b1'  # B4 → B1 (더 작고 빠름)
        self.IMG_SIZE = 256  # 380 → 256 (B1 적절한 크기)
        self.NUM_CLASSES = 17

        self.BATCH_SIZE = 24  # 16 → 24 (작은 모델이므로 배치 증가)
        self.VAL_BATCH_SIZE = 48
        self.TEST_BATCH_SIZE = 48
        self.TRAIN_WORKERS = 4
        self.EVAL_WORKERS = 4
        self.ACCUMULATION_STEPS = 2  # 3 → 2 (효과적 배치 48)
        self.EPOCHS = 25
        self.LR = 2e-4
        self.N_FOLDS = 5
...
def evaluate_holdout(holdout_df, fold_info, cfg):
    ...
    meta_df = pd.read_csv(f'{cfg.DATA_DIR}/meta.csv')

    print(f'\nHold-out Macro F1: {f1:.4f}')
    print('\n' + '=' * 90)
    print('클래스별 성능 (Hold-out)')
    print('=' * 90)
    print(f"{'클래스':<5} {'클래스명':<50} {'F1':<8} {'정확도':<12} {'샘플수':<8}")
    print('-' * 90)

    for class_id in range(cfg.NUM_CLASSES):
        class_name = meta_df[meta_df['target'] == class_id]['class_name'].values[0]
        f1_val = per_class_f1[class_id]

        class_mask = labels == class_id
        if class_mask.sum() > 0:
            accuracy = (final_preds[class_mask] == labels[class_mask]).mean()
            count = class_mask.sum()
            print(f"{class_id:<5} {class_name:<50} {f1_val:<8.4f} {accuracy*100:<11.2f}% {count:<8}")
        else:
            print(f"{class_id:<5} {class_name:<50} {f1_val:<8.4f} {'N/A':<12} {0:<8}")

    print('=' * 90)
    print(f"전체 Macro F1: {f1:.4f}")
    print('=' * 90)
